...
class XFeat(nn.Module):
	...
	def prepare_quant_model(self):
		...
		self.first_net = copy.deepcopy(self.net)
		# second model
		self.second_size = (1152, 1536)  # h,w
		self.second_net = copy.deepcopy(self.net)

		if not self.quant_s:
			input_first = torch.randn([1, 1, 512, 704], device=self.dev)
			unfold2d_first = torch.randn([1, 64, 64, 88], device=self.dev)
			self.first_quantizer = torch_quantizer(self.quant_mode, self.first_net, (input_first, unfold2d_first), device=self.dev, bitwidth=8, output_dir="first_model")
			quant_model = self.first_quantizer.quant_model
			self.first_net = quant_model.to(self.dev)			
		else:
			input_second = torch.randn([1, 1, 1152, 1536], device=self.dev)
			unfold2d_second = torch.randn([1, 64, 144, 192], device=self.dev)
			self.second_quantizer = torch_quantizer(self.quant_mode, self.second_net, (input_second, unfold2d_second), device=self.dev, bitwidth=8, output_dir="second_model")
			quant_model_second = self.second_quantizer.quant_model
			self.second_net = quant_model_second.to(self.dev)
 
	def export_qaunt_model(self):
		if self.quant_mode == 'calib':
			if not self.quant_s:
				self.first_quantizer.export_quant_config()
			else:
				self.second_quantizer.export_quant_config()
		if self.quant_mode == 'test':
			if not self.quant_s:
				self.first_quantizer.export_xmodel(deploy_check=True, output_dir="first_model")
				# self.first_quantizer.export_torch_script(output_dir="first_model")
				# self.first_quantizer.export_onnx_model(output_dir="first_model")
			else:
				self.second_quantizer.export_xmodel(deploy_check=True, output_dir="second_model")
				# self.second_quantizer.export_torch_script(output_dir="second_model")
				# self.second_quantizer.export_onnx_model(output_dir="second_model")
	...
	def extractDense(self, x, top_k = 8_000, is_first = True):
		...
		if self.quant:
			if is_first:
				...
				unfold2d_input = self.net._unfold2d(x, ws=8)
				M1, K1, H1 = self.first_net(x, unfold2d_input)
			else:
				x, rh1, rw1 = self.preprocess_tensor_quant(x, is_first = False)
				unfold2d_input = self.net._unfold2d(x, ws=8)
				M1, K1, H1 = self.second_net(x, unfold2d_input)
