...
def softmax_lut(kpts, LUT):
    ...
    lut_values = torch.tensor(LUT, dtype=torch.float32, device=device)[indices]
...
class XFeat(nn.Module):
    ...
    def __init__(self, top_k = 4096, detection_threshold=0.05):
        ...
        #self.Fast_MaxPool2d = FastMaxPool2d(kernel_size=(5,5), stride=1, padding=2)
    ...
    def get_kpts_heatmap(self, kpts, softmax_temp = 1.0):
        global sm_total_time_fast, sm_total_time_ref, sm_error
        # print('kpts.shape: ',kpts.shape)
        # for i in range(kpts.shape[2]):
        #     print(kpts[0,0,i])
        t1 = time.perf_counter()
        scores = F.softmax(kpts*softmax_temp, 1)[:, :64]
        # print('scores.shape: ',scores.shape)
        t2 = time.perf_counter()
        sm_total_time_ref += t2-t1
        t1 = time.perf_counter()
        # scores_lut = softmax_lut(kpts * softmax_temp, self.LUT)[:, :64]
        # print('scores_lut.shape: ',scores_lut.shape)
        t2 = time.perf_counter()
        sm_total_time_fast += t2-t1
        # sm_error += torch.abs(scores - scores_lut).mean().item()
        # print("Softmax time: ", t2-t1)
    ...
    def NMS(self, x, threshold = 0.05, kernel_size = 5):
        global total_time_fast, total_time_ref
        B, _, H, W = x.shape
        pad=kernel_size//2

        # t0 = time.perf_counter()
        local_max_ref = nn.MaxPool2d(kernel_size=kernel_size, stride=1, padding=pad)(x)
        # t1 = time.perf_counter()
        # total_time_ref += t1-t0

        # t0 = time.perf_counter()
        # local_max_fast = self.Fast_MaxPool2d(x)
        # t1 = time.perf_counter()
        # total_time_fast += t1-t0

        # diff = torch.abs(local_max_ref - local_max_fast).max()
        # if diff>1e-3:
        #     print("两种方法最大差异：", diff.item())
        
        pos = (x == local_max_ref) & (x > threshold)
    ...
    def get_NMS_total_time(self):
        return total_time_fast, total_time_ref
    
    def get_softmax_state(self):
        return sm_total_time_fast, sm_total_time_ref, sm_error
    ...
    def extractDense(self, x, top_k = 8_000):
        ...
        # self.net_time = self.net_time + time.time()-t
        # if self.count%4==0:
        #     NO = self.count/4
        #     print("\nNO.", NO, "net time =",self.net_time)
        #     self.net_time_avg = self.net_time_avg + self.net_time
        #     print("avg net time",self.net_time_avg / NO)
        #     self.net_time = 0;
